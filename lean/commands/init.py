...
def init(language: Optional[str]) -> None:
    ...
    # Create the config file (do not write job-organization-id; local-only mode)
    ...
    if cli_config_manager.default_language.get_value() is None:
        ...
        cli_config_manager.default_language.set_value(language)
...
- Run `lean data generate` to generate sample data, or add your own data to {DEFAULT_DATA_DIRECTORY_NAME}/
- Run `lean backtest "My Project"` with --data-provider-historical Local to backtest with local data
""".strip())
